...
namespace Server
{
    public partial class F_Server : Form
    {
        ...
        {
            lbl_Server.Text = "当前状态：启动中";
            btn_Server_Start.Text = "关闭服务";
            btn_Server_Start.Enabled = false;
            MessageBox.Show("为防止异常，若要关闭服务，请关闭窗口并输入管理员密码退出服务器！", "提示");
        ...
        {
            e.Cancel = true;
            lbl_warning.Text = "如要退出，请在退出验证中输入正确密码后退出";
        ...
        private void btn_download_Click(object sender, EventArgs e)
        {
            ...
            if (fullPath == "")
            {
                MessageBox.Show("文件为空！请点击下面的服务器文件目录选择一个文件！", "提示");
            }
            else
            {
                // 提取文件名
                ...
                if (cacheBuffer.Exsits(fileName))
                {
                    ...
                    MessageBox.Show("下载成功！本次从缓存中读取下载！下载位置为：C:\\Server_Download", "提示");
                }
                else
                {
                    //保存地址-创建前使用
                    ...
                    MessageBox.Show("下载成功！本次从磁盘中读取下载，并已缓存该文件！下载位置为：C:\\Server_Download_Disk", "提示");
                }
            }
        }
